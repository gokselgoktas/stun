...
/* Number of seconds that takes an empty loader task to complete.  This value
 * is used for dictating the minimum time spent showing the progress bars. */
#define MINIMUM_LOADER_TIME .25f
...
#define O7TNQ83D _(100.1)

/* The distance between the far and near planes. */
#define GUM5GQZZ _(99.9)
...
#define main(...)             \
    *JR031HK = (void *) 0x00; \
    int WINAPI WinMain(       \
        HINSTANCE IR5LT3IF, HINSTANCE Z7408PH4, LPSTR LKWCKNK, int N49XQUYK)
#endif /* FRAMEWORK == MINIMAL */
#endif /* SYSTEM == WINDOWS */
...
#define window(WIDTH, HEIGHT, IS_FULLSCREEN)                                   \
    ...
    if ((IS_FULLSCREEN)) {                                                     \
        ...
        ShowCursor(ChangeDisplaySettings(JNY8NEKN, CDS_FULLSCREEN));           \
    }                                                                          \
                                                                               \
    NEWWYUG8 = CreateWindow("static", NULL, WS_POPUP | WS_VISIBLE, 0, 0,       \
        (WIDTH), (HEIGHT), NULL, NULL, NULL, NULL);                            \
                                                                               \
    C98WXZEW = GetDC(NEWWYUG8);                                                \
    SetPixelFormat(C98WXZEW, ChoosePixelFormat(C98WXZEW, JNY8NEKN), JNY8NEKN); \
...
#define flush()                                             \
    if (PeekMessage(JNY8NEKN, NEWWYUG8, 0, 0, PM_REMOVE)) { \
...
#define window(WIDTH, HEIGHT, IS_FULLSCREEN)              \
    ...
    glfwOpenWindow((WIDTH), (HEIGHT), 8, 8, 8, 8, 32, 0,  \
        (IS_FULLSCREEN) ? GLFW_FULLSCREEN : GLFW_WINDOW); \
    RI1NFZQ4()
...
#define fill(POINTER, VALUE, SIZE) __builtin_memset((POINTER), (VALUE), (SIZE))
...
#else
/* The standard library flavor of fill. */
#define fill(POINTER, VALUE, SIZE) memset((POINTER), (VALUE), (SIZE))
...
#define allocate(SIZE) HeapAlloc(GetProcessHeap(), HEAP_ZERO_MEMORY, (SIZE))

/* The WinAPI flavor of dispose. */
#define dispose(POINTER) HeapFree(GetProcessHeap(), 0, (POINTER))
...
#define fill(POINTER, VALUE, SIZE) FillMemory((POINTER), (SIZE), (VALUE))

/* The WinAPI flavor of duplicate. */
#define duplicate(FROM, TO, SIZE) MoveMemory((TO), (FROM), (SIZE))
#endif /* FRAMEWORK == MINIMAL */
...
#define extension(TOKEN) ((PFNGL##TOKEN##PROC) R5UU5MEU[(TOKEN)])
...
static char const *const W2OPFFT9[NUMBER_OF_EXTENSIONS] = {
    ...
    "glCreateProgram", "glLinkProgram", "glUseProgram", "glGetUniformLocation",
    "glUniform1i", "glUniform1f", "glUniform1fv", "glUniform2f",
    "glUniformMatrix4fv", "glGenRenderbuffers", "glBindRenderbuffer",
    "glRenderbufferStorage", "glGenFramebuffers", "glBindFramebuffer",
    "glFramebufferTexture2D", "glFramebufferTextureLayer",
    "glFramebufferRenderbuffer", "glDrawBuffers", "glActiveTexture",
    "glTexImage3D", "glBufferData", "glBindBuffer", "glGenBuffers",

#if defined(EXTEND_RESOURCE_MANAGEMENT)
    "glDeleteShader", "glDeleteProgram", "glDeleteRenderbuffers",
...
#define J5F2LXAZ()                                             \
    do {                                                       \
        ...
        for (; Q1MVZ4Z3 < NUMBER_OF_EXTENSIONS; ++Q1MVZ4Z3) {  \
            R5UU5MEU[Q1MVZ4Z3] = MTO3ODSW(W2OPFFT9[Q1MVZ4Z3]); \
        }                                                      \
    } while (false)
...
typedef struct QE4KZPTZ accumulator_t;
typedef struct Y3JCIKIM layer_t;
typedef struct I3VGFDNN mesh_t;

/* Represents an accumulation layer.  An accumulation layer is a compound type
 * made out of a framebuffer and two textures attached to it, one for the color
 * products and one for the depth buffer. */
struct QE4KZPTZ {
    fbo_t framebuffer; /* FBO */
    texture_t attachments[2]; /* Render-products */

    unsigned int width; /* attachments' Width */
    unsigned int height; /* attachments' Height */
};
...
#define RI1NFZQ4()                                                         \
    do {                                                                   \
        ...
        while (true) {                                                     \
            ...
            glRectf(x, y, x + l * (512.f / MINIMUM_LOADER_TIME), y + 4.f); \
                                                                           \
            flush();                                                       \
        }                                                                  \
    } while (false)
    ...
    do {                                                                   \
        ...
        while (true) {                                                     \
            ...
            glRectf(x - 1.f, y - 1.f, x + 513.f, y + 5.f);                 \
                                                                           \
            glColor3ub(255, 255, 255);                                     \
            glRectf(x, y, x + (512.f - l * (512.f / MINIMUM_LOADER_TIME)), \
                y + 4.f);                                                  \
                                                                           \
            flush();                                                       \
        }                                                                  \
...
#define LINEAR_DEPTH(DEPTH) LIWQTX2O "/(" O7TNQ83D "-" DEPTH "*" GUM5GQZZ ")"

/* The accumulator function provides yet another special kind of layer; an FBO
 * with a predefined texture of the specified size attached to it, as well as a
 * depth texture. */
 ...
 * functionality provided by the blit function.
 *
 * Given an accumulator, this function blits the contents of the accumulator's
 * color render-product buffer to the provided destination layer, scaling the
...
#define gaussian_blur(LAYER, TEMPORARY)                                       \
    do {                                                                      \
        ...
        glUniform1f(glGetUniformLocation(FOBIMQQB[0], "m"), (LAYER)->height); \
        glUseProgram(FOBIMQQB[1]);                                            \
        glUniform1f(                                                          \
            glGetUniformLocation(FOBIMQQB[1], "m"), (TEMPORARY)->width);      \
                                                                              \
...
#define simplex_noise(LAYER, FREQUENCY, OCTAVES, AMPLITUDE)               \
    do {                                                                  \
        ...
        glUniform1f(glGetUniformLocation(FOBIMQQB[3], "f"), (FREQUENCY)); \
        glUniform1f(glGetUniformLocation(FOBIMQQB[3], "o"), (OCTAVES));   \
        glUniform1f(glGetUniformLocation(FOBIMQQB[3], "a"), (AMPLITUDE)); \
                                                                          \
